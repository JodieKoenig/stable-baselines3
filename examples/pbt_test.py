...
def train(ppo_agent, queue):
    train_agent = cloudpickle.loads(ppo_agent)
    train_agent.model, current_performance = train_agent.model.learn(
        total_timesteps=500, callback=eval_callback, reset_num_timesteps=False)
    send = cloudpickle.dumps(train_agent)
    queue.put(send)


def mod(test, nn, queue):
    test_check = cloudpickle.loads(test)
    print(test_check.model.learning_rate)
    print(test_check.model.num_timesteps)
    test_check.model.learn(total_timesteps=500)
    test_check.model.learning_rate = 10
    # print(test.num)
    # test.num = np.random.randn()
    # print(test.num)
    # test.name = nn
    send = cloudpickle.dumps(test_check.model)
    queue.put(send)
...
if __name__ == '__main__':
    ...
    for it in range(num):
        env = make_vec_env(env_id="FetchReachDense-v1", n_envs=1)
        envs.append(env)
        tests.append(Tester(tnum=it*2.0))
        # tests.append(Agents(env=envs[it], number=it))

    print('\n')
    workers = [multiprocessing.Process(target=train, args=(cloudpickle.dumps(test), out_queue)) for test in tests]
    ...
    for j in range(len(workers)):
        ...
        agent_changed = cloudpickle.loads(result)
        print(agent_changed.model.learning_rate)
        print(agent_changed.model.num_timesteps)
