...
import argparse
# from multiprocessing import Pool, Process, Queue, Lock, Manager, Value
...
def train(queue, ppo_agent, reward_threshold):
    ...
    queue.put(['hello', ppo_agent, ppo_agent.number, reward_threshold])
...
if __name__ == '__main__':
    ...
    # net = PPO("MultiInputPolicy", env, verbose=1).to(device)
    # net.share_memory()

    train_queue = mp.Queue(4)
    data_proc_list = []
    for _ in range(4):
        data_proc = mp.Process(target=train, args=(train_queue, device, train_queue))
        data_proc.start()
    ...
    for number in range(num_agents):
        env = make_vec_env(env_id="FetchReachDense-v1", n_envs=1)
        envs.append(env)
        agent = Value(Agents(env=envs[number], number=number))
        agents.append(agent)
