...
class Agents:
    def __init__(self):
        ...
        self.model = PPO(policy="MultiInputPolicy", env=env, verbose=1, n_steps=250)
        # put hyperparameters here instead?
        # learning rate, gamma, gae lambda?, clip range? ent coef?, vf coef?
        self.model.learning_rate = 0.003
        # self.model.learning_rate = random.randrange(0.000005, 0.003, 0.000001)
        self.performance = []
...
# env3 = make_vec_env(env_id="FetchReachDense-v1", n_envs=1)
# env4 = make_vec_env(env_id="FetchReachDense-v1", n_envs=1)
...
print(agents[0].env)
for agent in range(num_agents):
    agents[agent].env = envs[agent]

num_population_best_model = 0
num_population_worst_model = 0
...
threshold_rate = 0.2
current_threshold = -1     # worst
poor_reward = current_threshold - 2
...
if num_agents == 1:
    while not end_of_training:
        if flag_success == 0:
            ...
            agents[0].model, current_performance = agents[0].model.learn(
                total_timesteps=step_size, callback=eval_callback, reset_num_timesteps=False)
            agents[0].total_agent_timesteps = agents[0].model.num_timesteps
            agents[0].performance.append(current_performance)
            agents[0].latest_performance = current_performance
            ...
            if eval_callback.best_mean_reward > best_mean_reward:
                ...
                callback_on_best = StopTrainingOnRewardThreshold(reward_threshold=current_threshold, verbose=1)
                eval_callback = EvalCallback(envs[0], callback_on_new_best=callback_on_best, verbose=1,
                                             eval_freq=eval_frequency, n_eval_episodes=eval_episodes)

            overall_timesteps += agents[0].total_agent_timesteps

            pop_mean_reward, success_rate = evaluate_policy(
                agents[num_population_best_model].model, envs[num_population_best_model], n_eval_episodes=eval_episodes)
    ...
    while not end_of_training:
        if flag_success == 0:
            ...
            # if np.mean np.mean(agents[num].latest_performance) < poor_reward:
            #     agents[num].exploit = 1

                if eval_callback.best_mean_reward > best_mean_reward:
                    best_mean_reward = eval_callback.best_mean_reward
                    current_threshold = best_mean_reward + threshold_rate
                    callback_on_best = StopTrainingOnRewardThreshold(reward_threshold=current_threshold, verbose=1)
                    eval_callback = EvalCallback(envs[num], callback_on_new_best=callback_on_best, verbose=1,
                                             eval_freq=eval_frequency, n_eval_episodes=eval_episodes)
...
ax.plot(x1, y1)
fig.savefig("/Users/jodiekoenig/Documents/SkripsiePlotting/Test4_PBT/single_agent_smaller_nn.pdf", dpi=150)
record_video('FetchReachDense-v1', agents[num_population_best_model].model, video_length=500,
             prefix='pbt-single_agent_smaller_nn')
